...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// Because its located in the local scope
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(zooAnimals){
    let nameArr = [];
    zooAnimals.forEach(name => nameArr.push(`name: ${name.animal_name}, scientific: ${name.scientific_name}`));
    return nameArr;
  }
  ...
  */
    ...
    function lowPopulationAnimals(zooAnimals){
      return zooAnimals.filter(lowPop => {return(lowPop.population < 5)});
    }
 ...
 function USApop(zooAnimals){
  const x = zooAnimals.reduce((total,item) => {
    return total + item.population;},0);
return x;
  } 
